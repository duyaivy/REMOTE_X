...
public class ChatWindow {
    ...
    private void appendToPane(String name, String message, Style nameStyle, Style timeStyle) {
        // ✅ KIỂM TRA chatPane đã được khởi tạo chưa
        if (chatPane == null) {
            // Nếu cửa sổ chat chưa mở, không làm gì (hoặc lưu vào buffer nếu cần)
            System.out.println("[ChatWindow] Message received but window not shown: " + name + message);
            return;
        }
