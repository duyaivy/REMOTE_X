...
import java.io.DataOutputStream; // <-- THÊM IMPORT NÀY
...
public class Session {
    ...
    public synchronized void setViewerSocketAndAttemptRelay(Socket viewerSocket, String connectType) {
        ...
        // --- BƯỚC 5: Chờ đủ 6 socket (ĐÃ SỬA ĐỂ GỬI TÍN HIỆU "GO!") ---
        if (sharerScreenSocket != null && viewerScreenSocket != null &&
            ...
            // Sửa log
            System.out.println("All 6 sockets are connected. Sending START_SESSION signal...");
            this.relayStarted = true;
            
            try {
                // --- THÊM VÀO: GỬI TÍN HIỆU "GO!" ---
                // Gửi tín hiệu "Bắt đầu" cho Sharer (qua control socket)
                DataOutputStream sharerDos = new DataOutputStream(sharerControlSocket.getOutputStream());
                sharerDos.writeUTF("START_SESSION");
                sharerDos.flush();
                
                // Gửi tín hiệu "Bắt đầu" cho Viewer (qua control socket)
                DataOutputStream viewerDos = new DataOutputStream(viewerControlSocket.getOutputStream());
                viewerDos.writeUTF("START_SESSION");
                viewerDos.flush();
                // --- HẾT PHẦN THÊM ---
